...
def get_grad(vars, grid): # conformal version
  ...
  gradishs[0] = dxy_2nd_N(vars[0], grid)#, y_left= 1.0, y_right= 1.0)
  gradishs[1] = dxy_2nd_N(vars[1], grid)#, y_left= 1.0, y_right= 1.0)
...
def get_rates(vars, grid, mass = 1): # this version is for a conformal a and b for isotropic schwartzchild
  lapse, shift, f = vars[4] , vars[5], vars[6]
  ...
  f_1st = dxy_2nd_N(f, grid)#, y_left= 1.0, y_right= 1.0)
  f_2nd = dxy_2nd_N_2nd(f, grid)#, y_left= 1.0, y_right= 1.0)

  #rewrite a as a function of conformal stuff
  a = chi* f * vars[0]
  a_1st = chi_1st* f*vars[0] + chi*f*gradishs[0] + chi*f_1st*vars[0]

  #rewrite b as a function of conformal stuff
  b = chi* f * r_squared *vars[1]
  b_1st = (
    chi_1st * f * r_squared * vars[1]
    + chi * f_1st * r_squared * vars[1]
    + chi * f * (2 * grid * vars[1] + r_squared * gradishs[1])
  )

  b_2nd = (
    chi_2nd * f * r_squared * vars[1]
    + 2 * chi_1st * (
        f_1st * r_squared * vars[1]
        + f * (2 * grid * vars[1] + r_squared * gradishs[1])
    )
    + chi * (
        f_2nd * r_squared * vars[1]
        + 2 * f_1st * (2 * grid * vars[1] + r_squared * gradishs[1])
        + f * (
            2 * vars[1]
            + 4 * grid * gradishs[1]
            + r_squared * dxy_2nd_N_2nd(vars[1], grid)
        )
    )
  )
  ...
  #actual PDEs
  rates[0] = shift*(gradishs[0]) + shift_1st*((8/3)*vars[0]) - (4/(3*f))*lapse*(2*vars[2] + vars[3]*(vars[0]/vars[1]))
  rates[1] = shift*(gradishs[1] + 2*vars[1]/grid) + shift_1st*((2/3)*vars[1]) - ((2/(3*f)))*lapse*((vars[1]*vars[2]/vars[0])  + 5*vars[3])

  rates[2] = (
      lapse* (-(c*vars[2]/(f * vars[0])) + (2*vars[3]*c/(f * vars[1])) + (0.5*((b_1st*b_1st)/b + (a_1st*b_1st)/a) - (b_2nd))/b)
      + (2*shift_1st*c) + (shift * c_1st) - lapse_2nd + 0.5*(a_1st*lapse_1st/a)
  )
  rates[3] = (vars[2]*d*lapse/(f*vars[0])) + (1/a)*((0.25*a_1st*b_1st*lapse/a) - (0.5*b_2nd*lapse) - (0.5*lapse_1st*b_1st)) + lapse + shift*d_1st


  #rate of change of factor that is scaled by psi^4
  rates[6] = shift*(((chi_1st/chi)*f) + f_1st) + shift_1st*(-(2/3)*f) + (2/3)*lapse*((vars[2]/vars[0]) + 2*(vars[3]/vars[1]))
  ...
  K = (vars[2]/(vars[6]*vars[0])) + 2*(vars[3]/(vars[6]*vars[1]))

  b = vars[1]*vars[6]
  a = vars[0]*vars[6]
  c = vars[2]
  d = vars[3]
  rates[4] = - (2*lapse *K)# - shift*lapse_1st
  #rates[4] = (lapse_2nd - ((c*c/(a*a)) + 2*(d*d/(b*b)))*lapse - 15*K)*0.2

  b_a = (vars[1]/vars[0])
  #rates[5] = -(2/grid)*((b_a)**(2/3) - 1) - (2/3)*(((vars[0]*gradishs[1] - vars[1]*gradishs[0])/(2*vars[0]*vars[0]))* b_a**(-1/3))
  rates[5] = -(1 - b_a)
  return (rates)

def initial_vars(xnum, end = 100):
  vars = np.ndarray(shape = [7,xnum], dtype=np.double) # a, b, c and d are the "ith" row
  ...
  vars[6] = np.full(xnum, 1.0, dtype=np.double)#f
  return vars, x

# @title Constraints
def get_constaints(vars, grid, mass = 1): # this version is for a conformal b
  rates = np.ndarray(shape = [7,len(grid)], dtype=np.double)
  lapse, shift, f = vars[4] , vars[5], vars[6]
  rates = np.zeros_like(vars, dtype=np.double)
  ...
  f_1st = dxy_2nd_D(f, grid, y_left= 1.0, y_right= 1.0)
  f_2nd = dxy_2nd_D_2nd(f, grid, y_left= 1.0, y_right= 1.0)
...
def run(T_max, points, dt_dx_ratio = 0.1,saverate = 5, lengrid = 400):
  ...
  print('Run starting')
  print('Resolution: {0:.3f}M - Cauchy Factor: {1:.3f}'.format(x[2]-x[1], dt_dx_ratio))
  ...
  while (t < T_max):
    ...
    if t >= next_save:           # skip t = 0
        ...
        #T = t
        time.append(t)
        a.append(vars[0]*vars[6])
        b.append(vars[1]*vars[6])
        c.append(vars[2])
        d.append(vars[3])
        lap.append(vars[4])
        shit.append(vars[5])
  ...
  return x, a, b, c, d, lap, shit, lap_rate, shit_rate, momentum, hamiltonian, time, K
...
save_simulation_data(f'{filename}_3.pkl', simulation_data)